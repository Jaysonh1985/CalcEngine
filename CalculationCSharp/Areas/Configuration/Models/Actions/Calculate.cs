...
namespace CalculationCSharp.Areas.Configuration.Models.Actions
{
    public class Calculate
    {
        ...
        public void CalculateAction(List<CategoryViewModel> jCategory)
        {
            ...
            {
                foreach(var functionItem in group.Results)
                {
                    foreach (var item in functionItem.Functions)
                    {
                                    ...
                                    foreach (var param in item.Parameter)
                                    {
                                        ...
                                        else if (item.Function == "Function")
                                        {
                                            ...
                                            try
                                            {
                                                ...
                                                foreach(var functionRow in calcFunctionConfig[0].Results)
                                                {
                                                    foreach (var row in functionRow.Functions)
                                                    {
                                                        int index = parameters.Input[0].Results[0].Functions.FindIndex(a => a.Name == row.Name);
                                                        if (index >= 0)
                                                        {
                                                            row.Output = parameters.Input[0].Results[0].Functions[index].Output;
                                                        }
                                                        else
                                                        {
                                                            row.Output = null;
                                                ...
                                                foreach(var functionInput in calcFunctionConfig[0].Results)
                                                {
                                                    foreach (var input in functionInput.Functions)
                                                    {
                                                ...
                                                {
                                                    foreach(var row in col.Results)
                                                    {
                                                        int index = row.Functions.FindIndex(a => a.Function == "Return");
                                                        if (index >= 0)
                                                        {
                                                            item.Output = row.Functions[index].Output;
                                                            List<Return> returnList = new List<Return>();
                                                            foreach (var thing in row.Functions[index].Parameter)
                                                            {
